...
function MageBody() {
  ...
  useFrame((s) => {
    ...
    if (groupRef.current) {
      groupRef.current.position.y = 0.25 + Math.sin(t * 0.9) * 0.12
    }
    if (orbRef.current) {
...
export default function Mage({ nearPlayer, magePos }: Props) {
  return (
    <group position={magePos}>
      ...
      {/* Interaction prompt (floats above, always faces camera) */}
      {nearPlayer && (
