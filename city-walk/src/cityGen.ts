...
export interface PlacedBuilding {
  ...
  rotY: number
  scale: number
  hw: number  // AABB half-width
  hd: number  // AABB half-depth
}
...
// Building scale & layout constants
const SCALE     = 2.5    // world scale applied to models
const SLOT      = 5.8    // center-to-center distance between buildings (SCALE * 2 + gap)
const COLS      = 2      // buildings per block column
const ROWS      = 2      // buildings per block row
const BLOCK_W   = SLOT * COLS   // 11.6
const BLOCK_D   = SLOT * ROWS   // 11.6
const ROAD      = 7
const PITCH     = BLOCK_W + ROAD // 18.6
...
function generateCity() {
  ...
  for (let bx = 0; bx < GRID; bx++) {
    for (let bz = 0; bz < GRID; bz++) {
      ...
      // Street light at every intersection corner
      lights.push({ x: cx - PITCH / 2, z: cz - PITCH / 2 })

      // Zone from center determines building types
      const dMax = Math.max(Math.abs(bx - 2.5), Math.abs(bz - 2.5))

      let pool: string[]
      if (dMax < 1) {
        pool = [...SKYSCRAPERS, ...TOWERS]
      } else if (dMax < 2) {
        pool = [...COMMERCIAL, ...TOWERS]
      } else if (dMax < 2.5) {
        pool = [...COMMERCIAL, ...LOW_DETAIL]
      } else {
        pool = [...COMMERCIAL, ...HOUSES, ...LOW_DETAIL]
      }
      ...
      for (let row = 0; row < ROWS; row++) {
        for (let col = 0; col < COLS; col++) {
          ...
          // Scale variation: centre blocks slightly bigger
          const scaleVar = 0.9 + r() * 0.3
          const finalScale = SCALE * scaleVar
          // Half-size of 2×2 Kenney footprint at this scale
...
export function checkCollision(px: number, pz: number, radius = 0.5): boolean {
  for (const b of buildings) {
    // Swap hw/hd for 90° rotated buildings
    const swap = Math.round(b.rotY / (Math.PI / 2)) % 2 !== 0
    const [hw, hd] = swap ? [b.hd, b.hw] : [b.hw, b.hd]
    if (Math.abs(px - b.x) < hw + radius && Math.abs(pz - b.z) < hd + radius) {
      return true
    }
  }
  return false
